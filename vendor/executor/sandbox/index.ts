...
const sandboxDefectMessage = (cause: unknown): string => {
  if (cause !== null && typeof cause === "object") {
    ...
    if (
      tagged._tag === "ExecutionToolError" &&
...
const normalizeExecutionError = (
  ...
  return deadlineMs !== null &&
    Date.now() >= deadlineMs &&
    looksLikeInterruptedError(message)
...
const readOutputItems = (
  context: QuickJSContext
): ExecuteOutputItem[] | undefined => {
  const output = readPropDump(context, context.global, "__executor_outputs");
  return Array.isArray(output) && output.length > 0
    ? (output as ExecuteOutputItem[])
    : undefined;
};
...
const createLogBridge = (
  ...
  context.newFunction("__executor_log", (levelHandle, lineHandle) => {
    ...
    // Zoen: the log bridge allocates outside the WASM memory limit.
    if (logs.length < 64) logs.push(`[${level}] ${line.slice(0, 1024)}`);
...
const waitForDeferreds = async (
  ...
  const settled = Promise.race(
    [...pendingDeferreds].map((deferred) => deferred.settled)
  );
  const deadlineMs = deadline.deadlineMs();
  if (deadlineMs === null) {
    await settled;

...
const wrapAnonymousFunctionBody = (source: string): string =>
  `return await (${source})();`;
...
const renderExportDefaultBody = (
  ...
  if (declaration.type === "FunctionDeclaration") {
    ...
    return name
      ? wrapNamedFunctionBody(fnSource, name)
      : wrapAnonymousFunctionBody(fnSource);
  }
...
const renderHeuristicBody = (source: string): string => {
  const withoutDefaultExport = source
    .replace(/^export\s+default\s+/, "")
    .trim();

  if (
    (withoutDefaultExport.startsWith("async") ||
      withoutDefaultExport.startsWith("(")) &&

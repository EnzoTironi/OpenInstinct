...
export interface MailMessage extends Schema.Schema.Type<typeof MailMessage> {}
...
export interface MailboxSnapshot
  extends Schema.Schema.Type<typeof MailboxSnapshot> {}
...
export function parseMailbox(
  ...
  return Effect.gen(function* () {
    ...
    return { messages, ...(owner === undefined ? {} : { ownerEmail: owner }) };
  });
}
...
export const recentMessages = Effect.fn("Mailbox.recentMessages")(function* (
  ...
  return snapshot.messages.filter((message) => message.dateMs >= start);
});
...
export function ingestItems(
  ...
  return messages.map((message) => ({
    cc: message.cc.map((address) => address.email),
    date: message.dateMs,
    displayName: counterpart(message, ownerEmail).displayName,
    email: counterpart(message, ownerEmail).email,
...
function counterpart(message: MailMessage, ownerEmail?: string) {
  const people = participants(message, ownerEmail);
  return people[0] ?? message.from;
}

...
DO $verify$
BEGIN
  IF (SELECT count(*) FROM pg_database WHERE datname IN (current_database(), 'zoen_memory', 'zoen_matrix', 'zoen_whatsapp', 'zoen_vaultwarden')) <> 5 THEN
    RAISE EXCEPTION 'Application, memory, Matrix, WhatsApp and vault databases must all be restored';
  END IF;
  IF (SELECT count(*) FROM pg_roles WHERE rolname IN ('zoen_app','zoen_migrator','zoen_memory','zoen_matrix','zoen_whatsapp','zoen_vaultwarden')
      AND NOT rolsuper AND NOT rolcreatedb AND NOT rolcreaterole AND NOT rolreplication AND NOT rolbypassrls) <> 6
    ...
    OR has_database_privilege('zoen_whatsapp',current_database(),'CONNECT')
    OR has_database_privilege('zoen_vaultwarden',current_database(),'CONNECT')
    OR has_database_privilege('zoen_app','zoen_vaultwarden','CONNECT')
    OR has_database_privilege('zoen_app','zoen_whatsapp','CONNECT')
...
SELECT json_build_object(
  ...
  'amcheck', 'passed',
  'service_databases', json_build_array(current_database(), 'zoen_memory', 'zoen_matrix', 'zoen_whatsapp', 'zoen_vaultwarden'),
